document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('click', function (e) {
    ...
    var navbarEl = document.querySelector('.navbar, .nav, header');
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    if (isValid) {
      ...
      simulateFormSubmission(form, function (success) {
        if (success) {
          showFormMessage(form, form.getAttribute('data-success') || 'Your message has been sent successfully!', 'success');
          form.reset();
        } else {
          showFormMessage(form, form.getAttribute('data-error') || 'Something went wrong. Please try again.', 'error');
        }
  ...
  function validateField(field) {
    ...
    if (field.hasAttribute('required') && value === '') {
      isValid = false;
      errorMessage = field.getAttribute('data-required-msg') || 'This field is required.';
    } else if (value !== '') {
      if (type === 'email' || name === 'email') {
        var emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailPattern.test(value)) {
          isValid = false;
          errorMessage = field.getAttribute('data-email-msg') || 'Please enter a valid email address.';
        }
      }
      ...
      if ((type === 'tel' || name === 'phone' || name === 'tel') && value !== '') {
        ...
        if (!phonePattern.test(value)) {
          isValid = false;
          errorMessage = field.getAttribute('data-phone-msg') || 'Please enter a valid phone number.';
        }
      }
      ...
      if (field.hasAttribute('minlength')) {
        ...
        if (value.length < minLen) {
          isValid = false;
          errorMessage = field.getAttribute('data-minlength-msg') || 'Minimum ' + minLen + ' characters required.';
        }
      }
      ...
      if (field.hasAttribute('maxlength')) {
        ...
        if (value.length > maxLen) {
          isValid = false;
          errorMessage = field.getAttribute('data-maxlength-msg') || 'Maximum ' + maxLen + ' characters allowed.';
        }
      }
    }
  ...
  function clearFormErrors(form) {
    ...
    var existingMsg = form.querySelector('.form-status-message');
    if (existingMsg) {
  ...
  document.body.addEventListener('input', function (e) {
    ...
    if (field.classList.contains('field-error')) {
      ...
      var parent = field.parentNode;
      if (parent) {
        var errorMsg = parent.querySelector('.form-error-message');
        if (errorMsg) {
  ...
  function initScrollAnimations() {
    var targets = document.querySelectorAll(
      '.fade-in, .fade-up, .fade-down, .fade-left, .fade-right, [data-animate]'
    );

    if (targets.length === 0) {
      autoTagAnimatableElements();
      targets = document.querySelectorAll('[data-animate]');
    }
  ...
  function autoTagAnimatableElements() {
    var selectors = [
      '.hero-content',
      '.section-title',
      '.section-subtitle',
    ...
    selectors.forEach(function (selector, selectorIndex) {
      ...
      elements.forEach(function (el, index) {
        if (!el.hasAttribute('data-animate')) {
          ...
          el.setAttribute('data-delay', String((index % 4) * 100));
        }
      });
    });
  }
  ...
  var styleId = 'scroll-animation-styles';
  if (!document.getElementById(styleId)) {
    ...
    style.textContent = [
      ...
      '}',
      '.fade-in.pre-animate,',
      '[data-animate="fade-in"].pre-animate {',
      '  opacity: 0;',
      '}',
      '.fade-up.pre-animate,',
      '[data-animate="fade-up"].pre-animate {',
      '  opacity: 0;',
      '  transform: translateY(30px);',
      '}',
      '.fade-down.pre-animate,',
      '[data-animate="fade-down"].pre-animate {',
      '  opacity: 0;',
      '  transform: translateY(-30px);',
      '}',
      '.fade-left.pre-animate,',
      '[data-animate="fade-left"].pre-animate {',
      '  opacity: 0;',
      '  transform: translateX(30px);',
      '}',
      '.fade-right.pre-animate,',
      '[data-animate="fade-right"].pre-animate {',
      '  opacity: 0;',
      '  transform: translateX(-30px);',
      '}',
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '86314e8c-419e-438e-bb2a-315007a7454a',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '86314e8c-419e-438e-bb2a-315007a7454a';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '86314e8c-419e-438e-bb2a-315007a7454a';
